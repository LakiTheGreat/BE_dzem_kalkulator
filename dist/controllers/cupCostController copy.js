 ...
 */
export const getAllCupCosts = async (req, res) => {
    try {
 ...
 */
export const createCupCost = async (req, res) => {
 ...
 *         description: Internal server error
 */
export const putCupCost = async (req, res) => {
    const { id } = req.params;
 ...
 *         description: Internal server error
 */
export const deleteCupCost = async (req, res) => {
    const { id } = req.params;
    const cupCostId = Number(id);
    if (isNaN(cupCostId)) {
        res.status(400).json({ message: 'Invalid CupCost ID' });
...
//# sourceMappingURL=cupCostController%20copy.js.map