...
export const putOrder = asyncHandler(async (req: Request, res: Response) => {
  ...
  for (const item of cupData) {
    if (!userCupIds.includes(item.cupId)) {
      throw new AppError(
        `Invalid cupId ${item.cupId} for user ${userId} - putOrder`,
        status.BAD_REQUEST
      );
    }
  }
  ...
  const oldCups =
    (existingOrder.cups as unknown as { id: number; numberOf: number }[]) || [];
  const cupDiffs: { cupId: number; quantity: number }[] = [];
  ...
  if (cupDiffs.length > 0) {
    ...
    if (!existingInventory) {
      ...
      for (const { cupId, quantity } of cupDiffs) {
        ...
        if (existingCup) {
          existingCup.quantity += quantity;
          if (existingCup.quantity < 0) existingCup.quantity = 0; // no negative inventory
        } else {
          existingCupData.push({ cupId, quantity });
        }
      }
